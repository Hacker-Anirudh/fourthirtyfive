""" This does all the preprocessing work needed in one script! """
...
def main(project_root=None):
    """ Just the main function that runs when you run the program! """
    if project_root is None:
        resolved_project_root = Path(__file__).resolve().parents[2]
    else:
        resolved_project_root = Path(project_root)

    historical_dir = resolved_project_root / "src" / "model" / "data" / "historical"
    ...
    _copy_if_present(historical_dir / "state_pvi_midterms.csv",
                     processed_dir / "state_pvi_midterms.csv")
    _copy_if_present(historical_dir / "pres_approval.csv", processed_dir / "pres_approval.csv")
    _copy_if_present(historical_dir / "generic_ballot_midterms_1978_2022.csv",
                    processed_dir / "generic_ballot_midterms.csv")
